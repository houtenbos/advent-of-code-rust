...
fn _explore<'a>(graph: &UnGraphMap<&'a str, i32>, paths: &mut Vec<Vec<&'a str>>, path: &mut Vec<&'a str>, cave: &'a str, end: &'a str){
    for next_cave in graph.neighbors(cave){
        ...
        else if !(is_small_cave(next_cave) && path.contains(&next_cave) && check_double(&path) ) && next_cave != START {
            path.push(next_cave);
...
fn check_double(path: &Vec<&str>) -> bool {
    for i in 0..path.len(){
        if !is_small_cave(path[i]) {continue};
        for j in 0..path.len(){
            if i == j {continue};
            if path[i] == path[j] {
                return true;
